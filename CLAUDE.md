...
6. Floating chat widget: a chat bubble button fixed to the bottom-right corner, visible on every screen once a resume is loaded (not just inside a project), backed by `POST /chat`. If a project's detail view is currently open, the widget auto-scopes to that project (header reads "Chatting about: X") and includes the last-viewed concept, same as before; otherwise it falls back to general chat (header reads "General chat"), passing all initiative titles as context instead of one project's full detail. Each project keeps its own separate conversation thread, held in browser memory (`initiative._chatHistory`) and restored when you navigate back to it; general chat has its own separate thread (`state.generalChatHistory`). Nothing is persisted across a page refresh. Returns plain conversational text, not JSON — this is the one generation endpoint that isn't structured output.
...
9. Floating chat widget: `/chat` for free-form conversational Q&A, plain-text response, history + context kept in browser memory (done). Originally shipped as a fourth per-project tab (Study/Story/Q&A/Chat), then relocated to a global floating widget so it's usable from the dashboard too, not just inside a project — the tab was removed once the widget replaced it, not kept alongside it.
10. **(Phase 2, later)** Postgres schema in Supabase, wire extraction + generation output into the database, multi-resume support + `anon_id`, connect Supabase to the deployed backend
...
- Every generation prompt (Study, Story, Project Q&A, Resume Q&A, Chat) requires numbers/metrics to appear verbatim in the given context, with an explicit instruction to describe qualitatively rather than invent a plausible-sounding figure when no specific number is available. Chat's version of this rule had to explicitly call out "example answers you suggest the candidate could give" too — testing found the model would invent specific numbers (latency figures, dollar costs) when asked things like "how would an interviewer probe deeper," treating suggested sample answers as exempt from the rule until the prompt said otherwise.
...
- Chat's project context is the currently open initiative's own fields (+ last-viewed concept, if any) plus a one-line list of the candidate's *other* project titles — not the full resume text and not every other project's full detail. Keeps each call's context tight; when asked about a project that isn't the currently open one, the model is instructed to say it doesn't have those details loaded rather than guess. The floating widget derives "currently open project" from app state (`view === "detail"` and which initiative), not a separate tracked value — when no project is open (e.g. on the dashboard), it falls back to general chat with every initiative title listed as context instead of one project's full detail.
...
- `canopylabs/orpheus-v1-english` (Listen mode TTS) has an especially small free-tier budget — 3600 tokens **per day**, observed to cover roughly 3-4 full-length audio generations before failing. `/generate-audio` returns HTTP 429 with `"Daily audio quota reached. Try again later."` when this is hit (retries via `retry_on_rate_limit` don't help for this specific limit, since it resets daily, not per-minute) — this is expected and distinct from Groq's other, more generous per-minute/per-day limits on the text models
