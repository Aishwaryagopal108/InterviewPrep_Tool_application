...
6. Chat tab: a free-form conversational Q&A per initiative, backed by `POST /chat`. Takes the candidate's message, the conversation so far (held in browser memory per initiative, not persisted), the currently open initiative's fields, the last-viewed concept (if any), and a one-line list of the titles of the candidate's other projects for grounding without dumping full detail into every call. Returns plain conversational text, not JSON — this is the one generation endpoint that isn't structured output.
...
9. Chat tab: `/chat` for free-form per-project conversational Q&A, plain-text response, history + context kept in browser memory (done)
